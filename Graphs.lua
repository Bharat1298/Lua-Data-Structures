...
function Graph:new()
    return setmetatable({adjList = {}}, Graph)
end

function Graph:addEdge(u, v)
    if not self.adjList[u] then self.adjList[u] = {} end
    if not self.adjList[v] then self.adjList[v] = {} end
    table.insert(self.adjList[u], v)
    table.insert(self.adjList[v], u) -- Remove for directed graphs
end

function Graph:printGraph()
    for node, neighbors in pairs(self.adjList) do
        io.write(node .. ": ")
